...
class ShellCommand(Command):
    ...
    async def execute(self, args: argparse.Namespace) -> int:
        ...
        if use_ipython:
            ...
            self._run_ipython_subprocess(urls, banner)
        else:
            import code
    ...
    def _run_ipython_subprocess(self, url: dict[str,str], banner: str) -> None:
...
# if {repr(url)} or not ryx.is_connected():
#     asyncio.run(ryx.setup({repr(url)}))
