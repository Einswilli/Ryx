...
fn test_exact_filter() {
    ...
    let q = compile(&QueryNode::select("users").with_filter(FilterNode {
        field: "email".into(),
        lookup: "exact".into(),
        value: SqlValue::Text("alice@example.com".into()),
        negated: false,
    }))
    .unwrap();
...
fn test_negated_filter() {
    ...
    let q = compile(&QueryNode::select("posts").with_filter(FilterNode {
        field: "status".into(),
        lookup: "exact".into(),
        value: SqlValue::Text("draft".into()),
        negated: true,
    }))
    .unwrap();
...
fn test_contains_wraps_percent() {
    ...
    let q = compile(&QueryNode::select("posts").with_filter(FilterNode {
        field: "title".into(),
        lookup: "contains".into(),
        value: SqlValue::Text("rust".into()),
        negated: false,
    }))
    .unwrap();
    match &q.values[0] {
...
fn test_startswith_appends_percent() {
    ...
    let q = compile(&QueryNode::select("posts").with_filter(FilterNode {
        field: "title".into(),
        lookup: "startswith".into(),
        value: SqlValue::Text("Hello".into()),
        negated: false,
    }))
    .unwrap();
    match &q.values[0] {
...
fn test_endswith_prepends_percent() {
    ...
    let q = compile(&QueryNode::select("posts").with_filter(FilterNode {
        field: "title".into(),
        lookup: "endswith".into(),
        value: SqlValue::Text("World".into()),
        negated: false,
    }))
    .unwrap();
    match &q.values[0] {
...
fn test_q_not_tree() {
    init();
    let q_node = QNode::Not(Box::new(QNode::Leaf {
        field: "status".into(),
        lookup: "exact".into(),
        value: SqlValue::Text("draft".into()),
        negated: false,
    ...
    let q_node = QNode::And(vec![
        ...
        QNode::Or(vec![
            QNode::Leaf {
                field: "c".into(),
                lookup: "exact".into(),
                value: SqlValue::Int(3),
                negated: false,
            },
            QNode::Leaf {
                field: "d".into(),
                lookup: "exact".into(),
                value: SqlValue::Int(4),
                negated: false,
            },
        ]),
    ]);
...
fn test_left_outer_join() {
    ...
    let node = QueryNode::select("posts").with_join(JoinClause {
        ...
        table: "comments".into(),
        alias: None,
        on_left: "posts.id".into(),
        on_right: "comments.post_id".into(),
    });
    let q = compile(&node).unwrap();
...
fn test_order_asc() {
    init();
    let q =
        compile(&QueryNode::select("posts").with_order_by(OrderByClause::parse("title"))).unwrap();
...
fn test_order_desc() {
    init();
    let q =
        compile(&QueryNode::select("posts").with_order_by(OrderByClause::parse("-views"))).unwrap();
...
fn test_order_multiple() {
    ...
    let q = compile(
        &QueryNode::select("posts")
            ...
            .with_order_by(OrderByClause::parse("title")),
    )
    .unwrap();
...
fn test_delete_all_no_where() {
    ...
    assert!(
        ...
        "Delete all should have no WHERE: {}",
        q.sql
    );
}
...
fn test_custom_lookup_ilike() {
    init();
    lookups::register_custom("ilike", "{col} ILIKE ?").unwrap();
...
fn test_custom_lookup_tsearch() {
    ...
    let node = QueryNode::select("articles").with_filter(FilterNode {
        field: "body".into(),
        lookup: "tsearch".into(),
        value: SqlValue::Text("rust programming".into()),
        negated: false,
    });
    let q = compile(&node).unwrap();
...
fn test_unknown_lookup_errors() {
    ...
    let node = QueryNode::select("posts").with_filter(FilterNode {
        field: "title".into(),
        lookup: "nonexistent_xyz_lookup".into(),
    ...
    assert!(
        result
            .unwrap_err()
            .to_string()
            .contains("nonexistent_xyz_lookup")
    );
}
